...
model_s2.py
===========
Modelo S2 — Sombra Besseliana con Delta T Dinámico.
...
Física:
  ...
  - Tiempo: Incorpora la corrección Delta T = TT - UT1 interpolada dinámicamente
            desde las tablas oficiales del USNO (deltat.data y deltat.preds).
"""
...
def run_s2():
    ...
    print("\n[S2] Integrando órbita usando M5...")
    ...
    print("\n[S2] Proyectando sombra Besseliana con Delta T dinámico...")
    path_points = []
    ...
    for t_val, Y_val in zip(result["t_out"], result["Y_out"]):
        ...
        # S2: Delta T dinámico
        delta_t_s = get_delta_t(days_to_year_frac(t_val))
        t_ut1 = t_utc - timedelta(seconds=delta_t_s)
    ...
    print(f"\n======================================================================")
    print(f"  MODELO S2 — Sombra con Delta T Dinámico (interpolado) WGS84")
    print(f"======================================================================")
    ...
    if len(path_points) > 0:
        ...
        print(f"     Delta T interpolado: {best_pt['delta_t']:.2f} s")
